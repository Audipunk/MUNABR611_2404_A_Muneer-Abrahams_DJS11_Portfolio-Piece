...
import { Button, Dialog, Flex, Box, Text, Card, Tabs, ScrollArea } from "@radix-ui/themes";
import { PlayIcon, TrashIcon, ClockIcon, CheckCircledIcon } from "@radix-ui/react-icons";

function EpisodeCard({ episode, playbackPositions, handlePlayAudio }) {
        ...
        <Box pl="3" style={{ flex: 1 }}>
          ...
          {episode.completed ? (
            <Flex align="center" mt="1">
              ...
              <Text size="1" ml="1">
                Completed on: {new Date(episode.completedDate).toLocaleDateString()}
              </Text>
            </Flex>
          ) : (
            <Flex align="center" mt="1">
              ...
              <Text size="1" ml="1">
                Last played: {formatTime(playbackPositions[episode.id] || 0)}
              </Text>
            </Flex>
          )}
          <Flex mt="2" gap="2">
          ...
          <Tabs.Content value="started">
            <ScrollArea style={{ height: "600px" }}>
              {startedEpisodes.length === 0 ? (
                ...
                startedEpisodes.map((episode) => (
                  <EpisodeCard key={episode.id} episode={episode} playbackPositions={playbackPositions} handlePlayAudio={handlePlayAudio} />
                ))
              )}
            </ScrollArea>
          </Tabs.Content>
